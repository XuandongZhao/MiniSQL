...
bool RecordManager::selectop(TableNode& tablein, vector<AttriValue>*AttrSelected, vector<Condition>*ConditionAppied)
{
	...
	for (auto &i : tablein.AttributeList)
	{
		...
		{
			if (tablein.gettreeop(i.AtrributeName) != nullptr)
	...
	if ((*ConditionAppied).size() == 0)
	{
		...
		cout << "Selected using btree: " << res.size() <<"Time: "<<duration<< endl;
		return true;
	}
	else {
		if (bytree.empty())
		{
			...
			bool f= Select(tablein, AttrSelected, ConditionAppied);
			ed = clock();
			double duration = (double)(ed - st) / CLOCKS_PER_SEC;
...
int RecordManager::Delete(TableNode &tableIn, vector<Condition>*ConditionApplied)
{
	...
	while (totalRecordNum>0) {
		int delete_in_one_block = 0;
		int RecordNumEachBlock = *reinterpret_cast<int*>(block->cblock);///块头数量和首块信息会在删除的时候被修改
																		//	cout << RecordNumEachBlock << endl;
		totalRecordNum -= RecordNumEachBlock;
		/// while (RecordNumEachBlock>0){
		delete_in_one_block = SelectRecordEachBlock(tableIn, NULL, ConditionApplied, block, RecordNumEachBlock, enum_del);
		delete_amount += delete_in_one_block;
		if (delete_in_one_block > 0) {
			int flag = 0;
			queue<int> tmp_queue;
			int tmp = 0;
			while (mymap[tableIn.TableName].size() > 0) {
				tmp = mymap[tableIn.TableName].front();
				mymap[tableIn.TableName].pop();
				if (tmp != offset) {
					tmp_queue.push(tmp);
				}
				else {
					...
					while (tmp_queue.size()>0)
					{
						tmp = tmp_queue.front();
						tmp_queue.pop();
						mymap[tableIn.TableName].push(tmp);
					}
					break;
				}
			}
	...
	cout << "Number of tuples deleted: " << delete_amount << endl;
	return 1;
}
...
bool RecordManager::Insert(TableNode &tableIn, vector<AttriValue>*ValueToInsert)
{
	...
	while (mymap[fname].size()>0) {
		//cout << "map中未满块数目：" << mymap[fname].size() << endl;
...
int RecordManager::Select(TableNode &tableIn, vector<AttriValue>*AttrSelected, vector<Condition>*ConditionApplied)
{

	/*
	* Attrselect->遍历 push vector
	*
	...
	*/
	...
	while (totalRecordNum>0) {
		...
		totalRecordNum -= RecordNumEachBlock;
		///   while (RecordNumEachBlock>0){
		count += SelectRecordEachBlock(tableIn, AttrSelected, ConditionApplied, block, RecordNumEachBlock, enum_sel);
		///  }
		block = m.getblock(fileTableName, ++offset);
	}
	cout << "Number of tuples selected: " << count << endl;
	return count;
}
...
int RecordManager::SelectRecordEachBlock(TableNode &tableIn, vector<AttriValue>*AttrSelected, vector<Condition>*ConditionApplied, pointer block, int rnum, int op)
{
	if (block == NULL) {
		return false;
	}
	...
	if (op == enum_del && delete_amount > 0) {
		UpdateRecordNum(tableIn, block, delete_amount);
	}
	if (op == enum_sel) {
...
void RecordManager::DeleteInBTree(TableNode &tableIn, char *attriValBegin)
{
	...
	for (auto i = tableIn.AttributeList.begin(); i != tableIn.AttributeList.end(); i++) {
		if ((*i).isUnique || (*i).isPrimary) {
			...
			///cout << "delete in btree: " << content << endl; /*查看删除的属性值*/
			string keydel;
			if (bt->index.type == 2)
			{
				keydel = content;
...
bool RecordManager::ConditionSatisfied(TableNode &tableIn, char* recordBegin, vector<Condition>* ConditionApplied)
{
	...
	for (int i = 0; i<(*ConditionApplied).size(); i++) {
		char *contentAddress = recordBegin;///每次从attriList中第一个属性开始判断
...
bool RecordManager::ContentSatisfied(char* content, int type, Condition* condition)//////////改成了string
{
	if (type == 0) {
