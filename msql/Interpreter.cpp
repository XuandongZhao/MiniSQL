...
bool Interpreter::exec_create(std::string s, int &num)
{
	...
	if (word == "table")
	{
		...
		else
		{
			...
			return myapi.createtable(table_name, &av);
	...
	else if (word == "index")
	{
		...
		try
		{
			...
			if (word != ")")
				throw SyntaxException();
			return myapi.createindex(index_name, table_name, attribute_name);//do the create
			return 1;
		}
...
bool Interpreter::exec_select(std::string s, int &num)
{
	...
	{ //no confine
		...
		if (attribute_former.size() == 0) {
			...
			return myapi.selectrecord(table_name, &attribute_former, &condition_vector);
		}
		else {
			return myapi.selectrecord(table_name, &attribute_former, &condition_vector);//do the select
		}
		return 1;
	}
	else if (word == "where")
...
bool Interpreter::exec_drop(std::string s, int &num)
{
	...
	if (word == "table")
	{
		...
		if (!word.empty())
		{
			return myapi.droptable(word);//do the drop
			return 1;
		}
		else
		{
	...
	else if (word == "index")
	{
		...
		if (!word.empty())
		{
			return myapi.dropindex(word);//do the drop
			return 1;
		}
		else
		{
...
bool Interpreter::exec_delete(std::string s, int &num)
{
	...
	if (word.empty())
	{
		vector<Condition>* condition_delete_all = new vector<Condition>;
		return myapi.deleterecord(table_name, condition_delete_all);//do the delete
		return 1;
	}
	else if (word == "where")
	{
		...
		return myapi.deleterecord(table_name, &condition_vector);//do the delete
		return 1;
	}
	else
		return 1;
...
bool Interpreter::exec_insert(std::string s, int &num)
{
	...
	return myapi.insertrecord(table_name, &value);// do the insert
}
